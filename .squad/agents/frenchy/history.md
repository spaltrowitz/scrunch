...
- localStorage persistence was already implemented in a prior session (localProfile.ts, useUserProfile fallback, OnboardingWizard saving to localStorage, Dashboard/Recommendations/Profile auth gates removed). Key pattern: `scrunch_profile` stores the full Profile object, `scrunch_ratings` stores ratings as `Record<productId, 'loved'|'liked'|'ok'|'disliked'>`, `scrunch_actions` stores bookmarks/tried as `Record<productId, string[]>`, and `scrunch_notes` stores product notes
- For logged-out recommendations, `getLocalReviewsForRecs()` converts localStorage ratings into ProductReview-shaped objects so the same recommendation engine (buildTier1/2/3) works regardless of auth state
- The `useUserProfile` hook uses `queryKey: ['profile', userId ?? 'local']` with `enabled: true` always — when no userId, it returns `getLocalProfile()` from localStorage instead of hitting Supabase
- Mobile responsiveness pattern: stack layouts on mobile with `flex-col sm:flex-row`, use `grid-cols-1 sm:grid-cols-2` for grids, add `break-words` to long text to prevent horizontal scroll, `min-h-[44px]` for all touch targets per Apple/Android HIG guidelines (44px minimum), `py-2.5` on input fields = ~44px total height
- Rating button grids: change from `flex` (cramped 4-wide) to `grid grid-cols-2 sm:grid-cols-4 gap-2` for proper wrapping on narrow screens
- Footer links: add `flex-wrap` so links don't overflow on tiny screens (320px width)
- Auth loading gate: never return `null` from a route wrapper — render the app shell (Header/Footer) with a subtle placeholder so cold starts don't flash white. A `flex-1 animate-pulse bg-gray-50` div fills the content area without a jarring spinner.
- `useProduct(id)` placeholderData pattern: use `queryClient.getQueryData()` to search all product list caches (`['products']`, `['products','catalog']`, `['products','recommendations']`) before falling back to seedProducts. This makes Browse→Detail navigation instant with zero loading flash.
- React Query `placeholderData` can be a function — returning `undefined` means "no placeholder available" (query shows loading state as normal). Returning a value means the UI renders immediately while the real fetch runs in background.
